...
6. **`EVENTS_POLL_NAMES`** (или `events.poll.names`)
   - Описание: Упорядоченный список целей для бронирования (через запятую). Бот обрабатывает их **последовательно по приоритету**: пока текущая цель не «закрыта» (забронирована или нет свободных мест), следующие цели не рассматриваются.
   - Формат: `Название` или `Название:день`, где день — число 1–7 (понедельник–воскресенье, ISO). Сравнение по подстроке в названии, без учёта регистра.
   - Пример: `EVENTS_POLL_NAMES=Плавание:3,Плавание:1,теннис,Boxing (Красная Роза):2` — сначала плавание в среду, затем в понедельник, затем теннис, затем бокс во вторник.
   - Примечание: День недели определяется по полю `datesOf` события. Если событие с нужным днём ещё не появилось в списке API, цель остаётся активной.
...
- Цели из `EVENTS_POLL_NAMES` обрабатываются строго по порядку; переход к следующей — после успешной брони или если у всех подходящих событий нет свободных мест
- Сравнение названий событий происходит по подстроке (substring matching), регистр не учитывается
- Опциональный суффикс `:1`–`:7` фильтрует по дню недели (1 = понедельник, 3 = среда)
...
2. **`EVENTS_POLL_NAMES`** (или `events.poll.names`)
   - Упорядоченный список целей (последовательное бронирование). Опционально `:1`–`:7` — день недели (пн–вс).
   - Пример: `EVENTS_POLL_NAMES=Плавание:3,Плавание:1,теннис`
